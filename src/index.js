...
const updateProgressSubmitBtn = document.querySelector(
  "#update-progress-submit-btn"
);
...
const addBookToLibrary = (
  ...
  if (!title) {
    alert("Please input a title for the new book!");
    return;
  }
  if (!author) {
    alert("Please input an author for the new book!");
    return;
  }
  if (!pages) {
    alert("Please input a page count for the new book!");
    return;
  }
...
const getNewBookProgress = () => {
  ...
  for (let i = 0; i < newBookProgress.length; i++) {
    if (newBookProgress[i].checked) {
      newBookProgress = newBookProgress[i].value;
    }
  }
};
...
const updateProgress = () => {
  ...
  for (let i = 0; i < updateProgressSelection.length; i++) {
    if (updateProgressSelection[i].checked) {
      bookToUpdate.bookProgress = updateProgressSelection[i].value;
    }
  }
...
const displayAllCards = (book) => {
  ...
  newCardPageNumber.textContent = newBook.pageCount;
  ...
  if (newBook.bookProgress == 3) {
    addToColumn = finishedColumn;
  } else if (newBook.bookProgress == 2) {
    addToColumn = inProgressColumn;
  } else {
    addToColumn = notStartedColumn;
  }
...
const progressFormCloseBtnIcon = document.getElementById(
  "progress-form-close-icon"
);
...
function onBookFormSubmit() {
  if (!checkSignedInWithMessage()) {
    alert(`You must be signed in if you want your new book to be saved!`);
    return;
  }
  if (
    newTitle.value &&
